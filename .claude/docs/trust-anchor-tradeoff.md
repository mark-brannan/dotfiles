...
What stops a prompt-injected session that runs as the user and pushes a hook to
`main`:

- **unpinned `main`** — nothing stops it. Propagates in minutes.
- **file-based signing key** — nothing stops it either. The key lives in
  `$HOME`, and the session *is* the user. All the ceremony, none of the
  protection. This is the option to refuse.
- **SHA-in-form** — stops it. The bad commit sits inert until the user pastes a
