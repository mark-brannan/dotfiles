#!/bin/sh
# Blocks `git reset --hard` outright. The user ran one on a shared checkout to
# unwind her own bad commit and only found out afterwards; a hard reset
...
# through the harness. If it genuinely needs doing, the user runs it herself.
#
...
REASON='`git reset --hard` is blocked at user scope. It discards uncommitted work, and on a shared checkout that work may not be yours. Ask the user to run it themselves, or reach for a reversible move instead: `git revert`, a new branch off the good commit, `git stash`, or `git reset --soft`/`--mixed` if the working tree should survive.'
