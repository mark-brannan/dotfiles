...
Backlog for Claude-session work: infra, SignalK, code, docs. Sessions pull
from this board to start work and flush loose ends back to it at wrap-up.
The human team's board is `human.md` beside this file; physical boat work
lives in Evernote.
...
- **Wrap-up ritual:** on "wrap up" / "log it", flush loose ends here, append
  decisions to `maintenance/log.md`, commit, push, confirm the chat is safe
  to kill. (Push is part of the ritual — a handoff that only exists in a
  working tree has already been lost once.)
...
> Location provisional: Mark has placement concerns pending, so this file is
> staged in dotfiles `boards/` until that's settled. Migrated from symphony
> `maintenance/priorities.md` @ 97039c58e (2026-08-19); the shrink of that
> file happens after the location decision. Do not edit the symphony copy
> and this one divergently — this copy is authoritative for Claude work as
> of the migration.

## Rework follow-ups (this board's own bring-up)

- Settle board file location with Mark (his call; concerns pending)
- Shrink symphony `maintenance/priorities.md` once location lands (needs
  symphony push access)
- Update symphony `CLAUDE.md:45` "tracked separately" line to point at this
  board
- Delete `HANDOFF-board-rework.md` (dotfiles branch `claude/board-rework`)
