...
- `.local/bin/prune-branches` — deletes old local branches the remote already
  has (or has merged and dropped), across every repo on the machine; `/sweep`
  calls it, and `prune-branches.test.sh` beside it is its suite.
